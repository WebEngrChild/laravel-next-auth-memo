...
const Post: NextPage = () => {
  ...
  useEffect(() => {
    ...
    /* ToME
      ...
      cleanup関数は次のeffectが呼ばれる”前”や”アンマウント”する場合に呼ばれる
      今回の場合ではdependensyが空->初期レンダリング時にしか呼ばれない->アンマウントのみ
      またクリーンアップは前回の結果をリセットするものが本来の目的であるため、以下のような関数は特に意味をなさないと思われる。
      適当なvoid型の関数でも動作はする。
     */
        ...
        .post('/api/memos', data)
          ...
          .catch((err: AxiosError) => {
            ...
            if (err.response?.status === 422) {
              ...
              const validationMessages: { [index: string]: string } = {} as Validation; //asいらなくね？
